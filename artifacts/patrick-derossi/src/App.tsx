...
const GALLERY_SECTIONS = [
  ...
  { label: 'Double Storey', images: [] },
  { label: 'Developments', images: [] },
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: '#f4f5f8', color: '#1a1b1e', minHeight: '100vh' }}>
      ...
      <section id="work">
        <div className="container">
          ...
          {GALLERY_SECTIONS.filter(s => s.images.length > 0).map(section => (
            <div key={section.label} className="gallery-section reveal">
              <h3 className="gallery-section-heading">{section.label}</h3>
              <div className="gallery-columns">
                {section.images.map((src, i) => (
                  <div key={src} className="gallery-item" onClick={() => setLightbox(src)}>
                    <img src={src} alt={`${section.label} ${i + 1}`} loading="lazy" />
